...
import { 
  ...
  Tabs, Table, Tag, Typography, Space, Avatar, 
  Divider, Skeleton, message, Popconfirm 
} from "antd";
import { 
  UserOutlined, 
  LockOutlined, 
...
console.log(quotes);
  ...
  if (loading || userLoading) return <div style={{ padding: 100 }}><Skeleton active avatar /></div>;

  return (
    <Layout style={{ background: "#f0f2f5", padding: "40px 0", minHeight: "90vh" }}>
      <Content style={{ maxWidth: 1200, margin: "0 auto", width: "100%", padding: "0 20px" }}>
        <Row gutter={[24, 24]}>
          
          {/* LEFT SIDE: AVATAR & QUICK INFO */}
          <Col xs={24} lg={8}>
            <Card bordered={false} className="shadow-sm" style={{ textAlign: 'center', borderRadius: 12 }}>
              ...
              <Text type="secondary"><SafetyCertificateOutlined /> Đối tác doanh nghiệp</Text>
              ...
              <div style={{ textAlign: 'left' }}>
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  ...
                  <Text><HomeOutlined /> {user?.address || "Chưa cập nhật"}</Text>
                </Space>
              </div>

              <Popconfirm title="Bạn có chắc muốn đăng xuất?" onConfirm={handleLogout} okText="Đồng ý" cancelText="Hủy">
                <Button danger block icon={<LogoutOutlined />} style={{ marginTop: 24, borderRadius: 8 }}>
                  Đăng xuất tài khoản
                </Button>
              </Popconfirm>
            </Card>
          ...
          <Col xs={24} lg={16}>
            <Card bordered={false} className="shadow-sm" style={{ borderRadius: 12, minHeight: 500 }}>
              <Tabs defaultActiveKey="1" size="large" items={[
                {
                  key: '1',
                  label: <Space><UserOutlined />Hồ sơ của tôi</Space>,
                  children: (
                    <Form form={form} layout="vertical" onFinish={onUpdateProfile} style={{ marginTop: 20 }}>
                      <Row gutter={16}>
                        <Col span={12}>
                          <Form.Item label="Tên khách hàng / Doanh nghiệp" name="full_name" rules={[{ required: true }]}>
                ...
                {
                  key: '2',
                  label: <Space><FileSearchOutlined />Lịch sử báo giá</Space>,
                  children: (
                    <Table 
                      dataSource={quotes} 
                     ...
                     columns={[
                            {
                              ...
                              dataIndex: "id",
                              render: (id) => (
                                <Text code>{`SP${String(id).padStart(3, "0")}`}</Text>
                              ),
                            },
...

                          {
                            ...
                            render: (status) => {
                              const mapStatus = {
                                  ...
                                  shipping: { color: "processing", text: "Đang giao", icon: <CarOutlined /> },
                ...
                },
                {
                  ...
                  children: (
                    <Form form={pwdForm} layout="vertical" onFinish={onChangePassword} style={{ marginTop: 20, maxWidth: 400 }}>
